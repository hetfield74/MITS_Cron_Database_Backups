...
$lang_array = array(
  ...
  'TEXT_INFO_TASKS_MITS_CRON_DATABASE_BACKUPS' => 'Datenbanksicherung wird &uuml;ber die bestehende Callback-URL gestartet. Verbindungs-, HTTP- und Laufzeitfehler werden bei aktivierter Protokollierung im MITS-Log erfasst.',
  'BOX_MITS_CRON_DATABASE_RESTORE' => 'MITS Datenbankverwaltung',
  'BOX_MITS_CRON_DATABASE_TOOLS' => 'MITS Datenbankpflege',
);
...
if (defined('TABLE_CONFIGURATION') && function_exists('xtc_db_query') && function_exists('xtc_db_num_rows')) {
    ...
    if ($mits_sync_profile_check && xtc_db_num_rows($mits_sync_profile_check) > 0) {
        ...
        while ($mits_sync_profile = xtc_db_fetch_array($mits_sync_profile_query)) {
            ...
            $lang_array['TEXT_INFO_TASKS_' . $mits_sync_suffix] = 'Führt genau dieses MITS-Datenbank-Synchronisationsprofil aus. Intervall, Ankerzeit und Status können hier unabhängig von anderen Profilen geändert werden.';
        }
    }
}
